...
public class LogSerializationFactory {
	...
	private static final Logger LOGGER = LoggerFactory.INSTANCE.getLogger(LogSerializationFactory.class);
	...
	private LogSerializationFactory() {
		...
		FilterProvider filters = new SimpleFilterProvider().addFilter("alter output of filtered properties",
				new LogSerializationPropertyFilter());
	...
	public String getJsonString(Object json) {
		...
		try {
			...
			LOGGER.warn("Error logging JSON for class: {} with exception: {}", json.getClass().getName(), exception);
			return String.format("unable to log json class: %s", json.getClass().getName());
		}
	}

	private Map<String, AbstractLogSerializer> buildFieldSerializerMap(
			Map<Class<?>, Map<String, AbstractLogSerializer>> classSerializerMap, Class<?> jsonClass) {
		...
		for (Annotation annotation : field.getAnnotations()) {
			if (annotation.getClass().getInterfaces()[0].getSimpleName().contains("LogSerializer")) {
				try {
					...
					Class<AbstractLogSerializer> serializerClass = (Class<AbstractLogSerializer>) method
							.invoke(annotation);
